...
from core.models import PreAuthCaseInput, PreAuthSkillOutput  # noqa: E402


def run_engine_sync(case_input: PreAuthCaseInput) -> PreAuthSkillOutput:
    """Run the async engine synchronously for pytest sync fixtures."""
    ...
    if loop is not None:
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            future = executor.submit(
                asyncio.run, 
                run_engine(case_input)
            )
            return future.result()
    else:
        return asyncio.run(run_engine(case_input))