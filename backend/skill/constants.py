...
DEFAULT_MISTRAL_MODEL = environ.get("MISTRAL_MODEL", "mistral-small-latest")
MISTRAL_MODEL_CANDIDATES = [
    model.strip()
    for model in environ.get(
        "MISTRAL_MODEL_CANDIDATES",
        f"{DEFAULT_MISTRAL_MODEL},mistral-large-latest",
    ).split(",")
    if model.strip()
]
MISTRAL_MAX_OUTPUT_TOKENS = int(environ.get("MISTRAL_MAX_OUTPUT_TOKENS", "4500"))
...
MAX_RAW_CLINICAL_NOTES_LENGTH = int(
    environ.get("MAX_RAW_CLINICAL_NOTES_LENGTH", "60000")
)
