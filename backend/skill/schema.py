...
class ValidationInsights(BaseModel):
    """Input quality and completeness checks generated before adjudication."""
...
class AppealDraftResponse(BaseModel):
    """Structured appeal packet draft derived from adjudication output."""

    case_id: str
    recommendation: Literal["LIKELY_APPROVE", "NEED_MORE_INFO", "LIKELY_DENY"]
    summary_of_medical_necessity: str
    criterion_rebuttals: List[str]
    missing_evidence_checklist: List[str]
    requested_reconsideration_text: str
...
class PreAuthSkillOutput(BaseModel):
    ...
    validation_insights: Optional[ValidationInsights] = None
    consistency_score: Optional[float] = Field(default=None, ge=0.0, le=1.0)
    contradiction_risk_index: Optional[float] = Field(default=None, ge=0.0, le=1.0)
    appeal_readiness_score: Optional[float] = Field(default=None, ge=0.0, le=1.0)
