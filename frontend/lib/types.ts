...
export interface PreAuthSkillOutput {
  ...
  validation_insights?: ValidationInsights | null
  consistency_score?: number | null
  contradiction_risk_index?: number | null
  appeal_readiness_score?: number | null
  audit_flags?: string[]
...
// ── Training case (from GET /api/v1/samples) ──────────────
export interface TrainingCase {
  ...
  expanded_clinical_notes?: string
  has_full_packet?: boolean            // true for PA-001 only
}

// ── Schema field (from GET /api/v1/fields) ────────────────
...
// ── Validation (from GET /api/v1/benchmark) ───────────────
...
// ── Progress Event (from GET /api/v1/benchmark/stream) ───────
